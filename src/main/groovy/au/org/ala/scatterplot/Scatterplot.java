...
public class Scatterplot {
    ...
    void renderScatterplot(String filename_part) {
        ...
        try {
            ...
            File sessfolder = new File(dir);
            if (!sessfolder.exists()) {
                sessfolder.mkdirs();
            }

            imagePath = dir + uid + filename_part + ".png";
            imageURL = dir + uid + filename_part + ".png";
    ...
    private XYShapeRenderer getRenderer(LegendObject legend, int[] datasetColours, String[] seriesNames, double[] seriesValues) {
        ...
        int alpha = (int) (scatterplotStyleDTO.getOpacity() * 255);
    ...
    private void makeHtml() {
        ...
        for (int i = 0; i < layers.length - 1; i++) {
            for (int j = i + 1; j < layers.length; j++) {
                map.put(i + " " + j, pid + "_" + layers[i] + "_" + layers[j] + ".png");
                pos++;
            }
        }
        ...
        try {
            FileWriter fw = new FileWriter(dir + "scatterplot.html");
            fw.write(sb.toString());
            fw.close();
        } catch (Exception e) {
            e.printStackTrace();
        }
        logger.debug("ScatterplotList file: " + dir + "scatterplot.html");
